...
class _HouseholdPageState extends State<HouseholdPage> {
  ...
  Future<void> _save() async {
    ...
    final household = Household(
      ...
      street:
          _streetController.text.isEmpty ? null : _streetController.text.trim(),
      barangay:
    ...
    try {
      ...
      final currentUser =
          AuthService.getCurrentUser(); // You'll add this helper
  ...
  void _addFamilyMemberDialog({
    ...
    Uint8List? dialogImageBytes =
        existingMember?['philsys_image'] as Uint8List?;
                      ...
                      children: [
                        ...
                        if (educationStatus == 'Undergraduate') ...[
                          TextField(
                            ...
                            decoration: const InputDecoration(
                              labelText: 'Year Level (e.g., 1st Year)',
                            ),
                          ),
                          const SizedBox(height: 12),
                          TextField(
                            ...
                            decoration: const InputDecoration(
                              labelText: 'Course (e.g., BS Computer Science)',
                            ),
                          ),
                          const SizedBox(height: 12),
                        ],
                ...
                actions: [
                  ...
                  ElevatedButton(
                    onPressed: () {
                      ...
                      // Validation
                      if (educationStatus == 'Undergraduate' &&
                          (yearLevelCtrl.text.isEmpty ||
                              courseCtrl.text.isEmpty)) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text(
                              'Year Level and Course are required for Undergraduate',
                            ),
                          ),
                        );
                        return;
                      ...
                      if (educationStatus == 'Graduate' &&
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text(
                              'Please select Employed or Unemployed',
                            ),
                          ),
                        );
                        return;
  ...
  void _addEconomicDataDialog({
    ...
    final incCtrl = TextEditingController(
      text: existingData?['income']?.toString(),
    );
    ...
    showDialog(
      ...
      builder:
          (_) => AlertDialog(
            title: Text(
              existingData == null ? 'Add Economic Data' : 'Edit Economic Data',
            ),
            content: SingleChildScrollView(
              child: Column(
                mainAxisSize: MainAxisSize.min,
