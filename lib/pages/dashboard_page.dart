...
class _DashboardPageState extends State<DashboardPage>
    ...
    try {
      ...
      for (int year = 2015; year <= currentYear; year++) {
        ...
        for (var row in results) {
          hh++; // Count each household

          // Each household has 1 head of household
          int householdPopulation = 1;

          // Calculate population from family_members JSON
          final familyMembersJson = row[0]?.toString();
          if (familyMembersJson != null &&
              familyMembersJson.isNotEmpty &&
              familyMembersJson != 'null') {
            try {
              final List<dynamic> members = jsonDecode(familyMembersJson);
              householdPopulation +=
                  members.length; // Add family members to head of household
            } catch (e) {
              debugPrint('Error parsing family_members for year $year: $e');
              // If parsing fails, still count the head of household (already set to 1)
            }
          }

          pop += householdPopulation; // Accumulate population
        }
      ...
      // Get current year real stats - calculate from actual family members + head of household
      currentHouseholds = yearHouseholds[currentYear] ?? 0;

      // Calculate current population from all family members + head of household
      currentPopulation = await _calculateCurrentPopulation();

      // If calculation fails, fallback to yearPopulation
      if (currentPopulation == 0) {
        currentPopulation =
            yearPopulation[currentYear] ?? actualData.last.y.toInt();
      }

      // Projection stats
      projected2035 = (slope * 2035 + intercept).round();
      final growth10yr = projected2035 - currentPopulation;
      totalGrowthRate = ((growth10yr / currentPopulation) * 100)
          .toStringAsFixed(1);
      avgAnnualGrowth = ((growth10yr / 10) / currentPopulation * 100)
          .toStringAsFixed(2);
  ...
  /// Calculate current population from all households:
  /// - Count all family members from family_members JSON
  /// - Add 1 for each head of household
  Future<int> _calculateCurrentPopulation() async {
    try {
      final db = DBHelper.instance;

      // Fetch all households for current year
      final results = await db.query(
        '''
        SELECT 
          id,
      ...
      for (var row in results) {
        final String? familyMembersJson = row[2]?.toString();

        // Each household has exactly 1 head of household
        int householdCount = 1;

        // Parse and count family members from JSON
        if (familyMembersJson != null &&
            familyMembersJson.isNotEmpty &&
            familyMembersJson != 'null') {
          try {
            final List<dynamic> familyMembers = jsonDecode(familyMembersJson);
            householdCount += familyMembers.length;
          } catch (e) {
            // If JSON parsing fails, just count the head of household
            debugPrint('Error parsing family_members JSON: $e');
          }
        }
      ...
      debugPrint('Error calculating current population: $e');
      return 0;
    }
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: _loadAndPredictPopulation,
          ),
        ],
      ),
      body:
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          SizedBox(width: 10),
                          _buildStatCard(
                            "Households ($currentYear)",
                            currentHouseholds.toString(),
                            Colors.teal,
                          ),
                          SizedBox(width: 10),
                          _buildStatCard(
                            "Last Census Year",
                            actualData.last.x.toInt().toString(),
                            Colors.purple,
                          ),
                          SizedBox(width: 10),
                          _buildStatCard(
                            "Data Points Used",
                            actualData.length.toString(),
                            Colors.orange,
                          ),
                        ],
                      ),
                          ...
                          child: RichText(
                            text: TextSpan(
                              ...
                              children: [
                                ...
                                const TextSpan(
                                  text: " of actual census data from ",
                                ),
                                TextSpan(
                                  text:
                                      "${actualData.first.x.toInt()}–${actualData.last.x.toInt()}",
                                  style: const TextStyle(
                                    fontWeight: FontWeight.bold,
                                  ),
                                ),
                                const TextSpan(
                                  text:
                                      ", the population is projected to reach ",
                                ),
                                TextSpan(
                                  text: "$projected2035",
                                  style: const TextStyle(
                                ...
                                const TextSpan(
                                  text:
                                      " by 2035, with an average annual growth rate of ",
                                ),
                                TextSpan(
                                  text: "$avgAnnualGrowth%",
                                  style: const TextStyle(
                                    fontWeight: FontWeight.bold,
                                    color: Colors.green,
                                  ),
